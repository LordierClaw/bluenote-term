...
describe("TUI editor buffer adapter", () => {
  ...
  test("cuts, copies, and pastes through an injectable clipboard model", () => {
    ...
    const all = { start: 0, end: Array.from(editor.body).length, text: editor.body, collapsed: false }
  ...
  test("clipboard helpers expose desktop capability metadata without changing selected text edits", () => {
    const writes: string[] = []
    const clipboard: ClipboardModel = {
      name: "fake desktop clipboard",
      canRead: true,
      canWrite: true,
      readText: () => "Desktop paste",
      writeText: (text) => {
        writes.push(text)
      },
    }
    ...
    assert.equal(copySelection(editor, selection(6, 10), clipboard), "Beta")
    assert.deepEqual(writes, ["Beta"])

    const cut = cutSelection(editor, selection(6, 10), clipboard)
    assert.equal(cut.editor.body, "Alpha  Gamma")
    assert.deepEqual(writes, ["Beta", "Beta"])

    const pasted = pasteText(cut.editor, cut.selection, clipboard)
    assert.equal(pasted.editor.body, "Alpha Desktop paste Gamma")
  })
