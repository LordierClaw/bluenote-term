...
- **Manager** — a minimal Manager screen with a two-column browser/preview home screen backed by the same note list/selectors as the CLI. Its chrome shows useful context only: current folder path, the focused item/hovered path, and short action hints such as move, open, filter, back, and quit. Use right/open to enter a folder or open the selected note, and left/back to return to the previous folder or screen. `Escape` and `Ctrl+[` follow the same back rule for closing active modes and returning toward the root manager; quitting remains `q` or `Ctrl+C`.
