...
Phase 4E save behavior does not add a recovery-copy workflow. Atomic note-body writes may leave stale BlueNote-owned temp files after interruption; those temp files are internal implementation detail and are cleaned up silently when safe rather than promoted into user notes.
