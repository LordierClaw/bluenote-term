...
- **Manager** for two-column browser/preview note and folder navigation over existing note summaries and selectors. Phase 4F TUI cleanup/navigation/filtering/save is accepted and delivered: the Manager topbar shows filtered count plus app status, the footer names the currently opened note as `Currently open: <title>` or shows an empty/calm placeholder when no note is open, filtered count updates under filtering, and filtered result navigation opens the selected filtered result. Right/open enters folders or opens notes; left/back returns to the previous folder or screen; `/` filters the current folder; `n` creates a new note; `d` deletes the focused note only after confirmation; `Ctrl+P` opens Search Everything; `Esc` backs toward Manager/root.
- **Editor** for focused inline editing of plain Markdown note bodies with top/bottom status chrome, live typing/input regression coverage, `Ctrl+F` find mode, `Ctrl+R` replace mode, and 750ms autosave guarded against stale completion. Phase 4E autosave atomicity is accepted and delivered: autosave and manual `Ctrl+S` use the same safe note-body write path, failed saves keep the buffer dirty and retry later, no recovery-copy workflow is created, and stale temp files are a BlueNote-owned internal implementation detail. Phase 4F editor border is removed and the inner `Editor body` title is removed; the editor topbar shows note name, path, and modified time, while the bottom bar shows `Line`/`Col`, wrap mode, save/autosave status, and shortcuts.
...
Phase 4E save behavior does not add a recovery-copy workflow. Atomic note-body writes may leave stale BlueNote-owned temp files after interruption; those temp files are internal implementation detail and are cleaned up silently when safe rather than promoted into user notes. Phase 4F real TTY smoke covers autosave and manager switching after edit by returning from an autosaved editor to Manager and opening another note with Enter and Arrow Right.

Current TUI bootstrap status is `phase-4f-tui-cleanup-navigation-save-bugs`; the next marker remains the neutral `phase-4-next-hardening-subplan`.
