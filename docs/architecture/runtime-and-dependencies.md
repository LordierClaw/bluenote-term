...
- **Manager** for two-column browser/preview note and folder navigation over existing note summaries and selectors. Phase 4F TUI cleanup/navigation/filtering/save is accepted and delivered: the Manager topbar shows filtered count plus app status, the open-note bottom path is exactly the currently opened note full path, or an empty/calm placeholder when no note is open, filtered count updates under filtering, and filtered result navigation opens the selected filtered result. Right/open enters folders or opens notes; left/back returns to the previous folder or screen; `n` creates a new note; `d` deletes the focused note only after confirmation.
