...
- `bluenote-v0.1.0-windows-x64.zip` — Windows x64 portable ZIP containing `bn.exe`, `sql-wasm.wasm`, and usage notes.
- `bluenote-v0.1.0-linux-x64.tar.gz` — Linux x64 portable tarball containing `bn`, `sql-wasm.wasm`, and usage notes.
...
1. Download `bluenote-v0.1.0-windows-x64.zip` from the GitHub Release.
2. Extract the ZIP.
...
Download `bluenote-v0.1.0-linux-x64.tar.gz`, then run:

```bash
tar -xzf bluenote-v0.1.0-linux-x64.tar.gz
cd bluenote
./bn --help
./bn init
./bn tui
...
```bash
sha256sum bluenote-v0.1.0-linux-x64.tar.gz
```

On Windows PowerShell, compare the printed hash with the matching checksum in the GitHub Release notes:

```powershell
Get-FileHash .\bluenote-v0.1.0-windows-x64.zip -Algorithm SHA256
```
