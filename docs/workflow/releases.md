...
- `bluenote-v0.4.0-windows-x64.zip` — Windows x64 portable ZIP containing `bn.exe`, `sql-wasm.wasm`, and usage notes.
- `bluenote-v0.4.0-linux-x64.tar.gz` — Linux x64 portable tarball containing `bn`, `sql-wasm.wasm`, and usage notes.
...
1. Download `bluenote-v0.4.0-windows-x64.zip` from the GitHub Release.
2. Extract the ZIP.
...
Download `bluenote-v0.4.0-linux-x64.tar.gz`, then run:

```bash
tar -xzf bluenote-v0.4.0-linux-x64.tar.gz
cd bluenote
./bn --help
./bn init
./bn tui
...
```bash
sha256sum bluenote-v0.4.0-linux-x64.tar.gz
```

On Windows PowerShell, compare the printed hash with the matching checksum in the GitHub Release notes:

```powershell
Get-FileHash .\bluenote-v0.4.0-windows-x64.zip -Algorithm SHA256
```
...
For a manual dry run from GitHub, use the `workflow_dispatch` trigger on the release workflow and leave `release_version` at the current package version (for example `v0.4.0`). A dispatch run builds and uploads workflow artifacts using the same versioned archive names; publishing a GitHub Release still requires pushing a `v*` tag.
