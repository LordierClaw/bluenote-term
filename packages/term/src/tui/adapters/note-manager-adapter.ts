import { containsSearchQuery, type NoteSummary, type ShowNoteSummary } from "@lordierclaw/bluenote-core"
...
function folderAncestorsFor(folderPath: string): string[] {
  const normalizedPath = normalizeRelativePath(folderPath).replace(/^\/+|\/+$/gu, "")

  if (!isUserNoteFolderPath(normalizedPath)) {
    return []
  }
...
function allBrowserItems(noteSummaries: readonly NoteManagerSummary[], userFolderPaths: readonly string[] = []): ManagerItem[] {
  const folderPaths = new Set<string>()
  const noteItems: ManagerItem[] = []

  for (const folderPath of userFolderPaths) {
    for (const userFolderPath of folderAncestorsFor(folderPath)) {
      folderPaths.add(userFolderPath)
    }
  }
...
export function buildManagerBrowserItems(noteSummaries: readonly NoteManagerSummary[], userFolderPaths: readonly string[] = []): ManagerItem[] {
  return allBrowserItems(noteSummaries, userFolderPaths).map(cloneItem)
}
...
export function buildManagerBrowserModel(
  ...
  const items = allBrowserItems(noteSummaries, options.userFolderPaths)
